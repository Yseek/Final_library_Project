	...
	return (
		<center>
			...
			{pageList.length !== 0 && <div className="paging">
				...
				<span><Link to={`/admin/notice/${Math.max(1, page.page - pageWidth)}`} className="btn-paging prev">&lt;</Link></span>&nbsp;
				{pageList.map(res => (
					<span key={res}>
				...
				<span><Link to={`/admin/notice/${Math.min(page.totalPages, page.page + pageWidth)}`} className="btn-paging next">&gt;</Link></span>&nbsp;&nbsp;
